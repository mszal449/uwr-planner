...
export async function getAll<T>(name: string): Promise<DataI<T>> {
    const response = await fetch(`${process.env.URL}/api/${name}`)
    if(!response.ok) {
...
export async function getWithFilters<T>(name: string, filters: Record<string, any>): Promise<DataI<T>> {
    ...
    const response = await fetch(`${process.env.URL}/api/${name}?${searchParams.toString()}`);
    if (!response.ok) {
        throw new Error(`Error: status ${response.status}`);
    }
    const data = await response.json();
...
export async function getById<T>(name: string, id: string): Promise<T | null> {
    const response = await fetch(`${process.env.URL}/api/${name}/${id}`);
    if (!response.ok) {
        return null;
    }
    const data = await response.json();
