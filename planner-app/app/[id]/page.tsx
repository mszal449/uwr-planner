...
export default function PlanView({params}: {params: {id: string}}) {
  ...
  function addSelectedCourse(semId: number) {
    if (plan && selectedCourse) {
      plan.semesters[semId].push(selectedCourse)
    ...
    <div className="flex flex-col h-screen "> 
      <div className="flex-grow grid grid-cols-12 gap-4 p-5 pb-0 min-h-0">
        <CourseBrowser 
          ...
          onSelectCourse={(c: CourseI) => {
            setSelectedCourse(c)
            console.log(`yipeee course ${c.name} selected!!!!`)
            }} />
        <SemesterPlanner 
          styles="col-start-3 col-end-13" 
          plan={plan}
          onSelectSemester={(semId: number) => {
            addSelectedCourse(semId)
            console.log(`Uhm semester ${semId} selected`)}}/>
      </div>
